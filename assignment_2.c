...
void Display_inventory(int product_code[], int quantity_in_stock[], int price_per_product[], int nproducts) {
    int i;
    printf("\nTHE LIST OF THE PRODUCTS INFORMATION ARE :\n");
    printf("\nPRODUCT CODE     |QUANTITY IN STOCK     |PRICE PER PRODUCT");
    for (i = 0; i < nproducts; i++) {
        printf("\n%d     |%d     |%d" , product_code[i] , quantity_in_stock[i] , price_per_product[i]);
    }
}
...
int main() {
    ...
    while (flag == 1) {
        printf("\n\n--- STACK MENU ---\n");
        printf("1 to Insert (Push)\n");
        printf("2 to Remove (Pop)\n");
        printf("3 to Display Top (Peek)\n");
        printf("4 to Show All Elements\n");
